...
import {
  ...
  DialogTitle, FormControl, IconButton, InputLabel, MenuItem, Select, Slider,
...
// Mêmes catégories et mêmes widgets d'affinage que le panneau de la carte
// mentale : slider / périodes nommées, GeoNames, IdRef, vocabulaires contrôlés.
...
export default function AddThemeDialog({ open, onClose, onAdd }: Props) {
  ...
  // État de l'éditeur inline (une catégorie ouverte à la fois, comme la carte)
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {CATEGORIES.map(({ key, label: catLabel, Icon, color }) => {
            const items = (attrs[key] ?? []) as AttrItem[]
