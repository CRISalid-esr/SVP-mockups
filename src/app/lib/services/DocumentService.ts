...
export class DocumentService {
  ...
  private getMockData<T>(filename: string): T {
    const filePath = path.join(process.cwd(), 'src', 'mocks', 'data', filename)
  ...
  async fetchDocuments({
    ...
    if (this.isMock()) {
      const allMockDocuments = this.getMockData<any[]>('documents.json').map(
        Document.fromJson,
      )
      let filtered = allMockDocuments

      if (searchTerm) {
        const lowerSearch = searchTerm.toLowerCase()
        filtered = filtered.filter((doc) =>
          doc.titles.some((t) => t.value.toLowerCase().includes(lowerSearch)),
        )
      }
      ...
      return { documents, totalItems: filtered.length }
    }
  ...
  async countDocuments({
    ...
    if (this.isMock()) {
      const allMockDocuments = this.getMockData<any[]>('documents.json')
      return {
        allItems: allMockDocuments.length,
        incompleteHalRepositoryItems: allMockDocuments.filter(
          (d) => !d.halCollectionCodes?.length,
        ).length,
      }
    }
  ...
  async fetchDocumentById(uid: string) {
    if (this.isMock()) {
      ...
      const doc = allMockDocuments.find((d) => d.uid === uid)
      if (!doc) throw new Error('Document not found in mock data')
      return doc
    }
    try {
